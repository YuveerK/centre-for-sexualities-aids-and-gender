...
const quickActions = [
  { title: 'Our Story', text: 'History, people, and the CSA&G journey', cta: 'Read More' },
  { title: 'Working with Young People', text: 'Youth-centred engagement for social justice', cta: 'Read More' },
  { title: 'Key Projects', text: 'Flagship initiatives advancing equity', cta: 'Read More' },
  { title: 'Publications', text: 'Research outputs and learning tools', cta: 'Browse' },
]
...
const signaturePrograms = [
  {
    ...
    text:
      'A flagship volunteer and leadership development programme building a movement of active citizen student leaders.',
  },
  {
    title: 'Africa-Europe CoRE HGS',
    text:
      'Interdisciplinary research cluster advancing evidence-based policy and equitable collaboration in health, gender, and sexualities.',
  },
  {
    title: 'GBV & Social Justice',
    text:
      'A five-year collaborative programme supporting partners to strengthen evidence-based practice and inclusive advocacy.',
  },
  {
    title: 'Gender Equality Changemakers',
    text:
      'A professional certification pathway equipping gender equity change agents with applied leadership and organisational skills.',
  },
]

const selectedPublications = [
  {
    title: '(Un)knowing MEN: Africanising gender justice programmes for men in South Africa',
    meta: '2018 · CSA&G Press',
  },
  {
    title: 'Women in the Context of Justice: Continuities and Discontinuities in Southern Africa',
    meta: '2018 · CSA&G Press',
  },
  {
    title: 'Integrating HIV and AIDS into the curriculum at the University of Pretoria: Time for transformation?',
    meta: '2016 · CSA&G',
  },
]
...
const newsItems = [
  ...
  {
    title: 'Call for Applications: One-Year International Research and Advocacy Exchange',
      ...
      <header className="relative overflow-hidden bg-ink-950 text-white">
        ...
        <div className="relative z-10 mx-auto grid max-w-6xl gap-10 px-6 pb-24 pt-16 lg:grid-cols-[1.2fr_0.8fr] lg:px-10">
          <div className="space-y-8">
            <p className="section-kicker text-white/70">Centre for Sexualities, AIDS & Gender</p>
            ...
            <p className="max-w-xl text-lg text-white/80">
              The CSA&G is a University of Pretoria centre advancing evidence, practice, and policy across gender,
              sexualities, HIV, and social justice. We collaborate locally and globally to shape safer, fairer futures.
            </p>
            <div className="flex flex-wrap gap-4">
              <button className="rounded-full bg-gold-500 px-6 py-3 text-sm font-semibold text-ink-950 shadow-glow">
                Explore our work
              </button>
          ...
          <div className="space-y-6">
            {heroImages.map((image) => (
              <div key={image.src} className="overflow-hidden rounded-3xl border border-white/10 bg-white/5 shadow-lift">
      ...
      <section className="relative mx-auto max-w-6xl px-6 py-16 lg:px-10">
        <div className="grid gap-6 lg:grid-cols-4">
          {quickActions.map((action) => (
            <div key={action.title} className="rounded-3xl border border-ink-900/10 bg-white p-6 shadow-lift">
              <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-ink-950 text-xs font-semibold text-white">
                CSA
              </div>
              <h3 className="mt-6 font-display text-xl text-ink-950">{action.title}</h3>
              <p className="mt-3 text-sm text-ink-700">{action.text}</p>
              <button className="mt-6 text-sm font-semibold text-ink-950">{action.cta}</button>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-white">
        <div className="mx-auto grid max-w-6xl gap-12 px-6 py-20 lg:grid-cols-[1.1fr_0.9fr] lg:px-10">
          <div className="space-y-6">
            ...
            <h2 className="section-title">Understanding power, exploring diversity, enabling inclusivity.</h2>
            <p className="text-ink-700">
              We advance research and practice that engage individual behaviour, social meaning, and structural
              enablers and barriers. Our work supports inclusive sexual citizenship and active accountability.
            </p>
            <div className="grid gap-4 sm:grid-cols-2">
              {impactAreas.map((theme) => (
                <div key={theme} className="rounded-2xl border border-ink-900/10 bg-sand-50 p-4 text-sm">
                  {theme}
                </div>
              ))}
            </div>
          ...
          <div className="rounded-3xl border border-ink-900/10 bg-sand-50 p-8">
            ...
            <div className="mt-6 space-y-5">
              {signaturePrograms.map((program) => (
                <div key={program.title} className="rounded-2xl border border-ink-900/10 bg-white p-5">
        ...
        <div className="mx-auto max-w-6xl px-6 py-20 lg:px-10">
          <div className="grid gap-10 lg:grid-cols-[1.2fr_0.8fr]">
            <div className="space-y-6">
              ...
              <h2 className="section-title">CSA&G Press and knowledge products</h2>
              <p className="text-ink-700">
                Peer-reviewed and practice-oriented outputs that shape policy, programmes, and public understanding
                across gender, sexualities, and social justice.
              </p>
              <button className="rounded-full bg-ink-950 px-6 py-3 text-sm font-semibold text-white">
                Browse catalogue
              </button>
            </div>
            <div className="space-y-4">
              {selectedPublications.map((pub) => (
                <div key={pub.title} className="rounded-3xl border border-ink-900/10 bg-white p-6 shadow-lift">
                  <p className="font-semibold text-ink-950">{pub.title}</p>
                  <p className="text-sm text-ink-700">{pub.meta}</p>
                  <button className="mt-4 text-sm font-semibold text-ink-950">Download</button>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="bg-white">
        <div className="mx-auto max-w-6xl px-6 py-20 lg:px-10">
          <div className="flex flex-wrap items-end justify-between gap-6">
            ...
            <button className="rounded-full border border-ink-900/20 px-5 py-2 text-sm font-semibold text-ink-900">
              View archive
            </button>
          </div>
          <div className="mt-10 grid gap-6 lg:grid-cols-2">
            {newsItems.map((item) => (
              <div key={item.title} className="rounded-3xl border border-ink-900/10 bg-sand-50 p-6 shadow-lift">
                ...
                <button className="mt-6 text-sm font-semibold text-ink-950">Read more</button>
              </div>
            ))}
          </div>
            ...
            <div className="fade-edge overflow-hidden">
              <div className="flex w-[200%] animate-marquee gap-6 text-sm text-ink-700">
                {[...newsItems, ...newsItems].map((item, index) => (
                  <span key={`${item.title}-${index}`} className="whitespace-nowrap">
      ...
      <section className="bg-ink-950 text-white">
        <div className="mx-auto grid max-w-6xl gap-10 px-6 py-20 lg:grid-cols-[1.1fr_0.9fr] lg:px-10">
          <div className="space-y-6">
            ...
            <h2 className="section-title text-white">Partner with the centre</h2>
            ...
            <div className="flex flex-wrap gap-3 text-sm">
              {['Facebook', 'X', 'YouTube', 'Instagram', 'Soundcloud'].map((social) => (
                <button
                  key={social}
          ...
          <div className="rounded-3xl border border-white/10 bg-white/5 p-8">
            ...
            <div className="mt-6 grid gap-4">
              ...
              <button className="rounded-full bg-gold-500 px-5 py-3 text-sm font-semibold text-ink-950">
                Submit message
              </button>
            </div>
          </div>
