...
const getDateKey = (article) => {
  if (article.datePublished) {
    ...
    if (!Number.isNaN(date.getTime())) {
      ...
      return { year, month, key: `${year}-${String(date.getMonth() + 1).padStart(2, '0')}` }
    }
  }
...
export default function News() {
  ...
  const articlesWithDate = useMemo(
    () =>
      articles.map((article) => {
        ...
        const monthIndex =
          year === 'Unknown' ? '00' : String(new Date(`${month} 1, ${year}`).getMonth() + 1).padStart(2, '0')
        return { ...article, _year: year, _month: month, _monthIndex: monthIndex, _key: key }
      }),
    [],
  )

  const availableYears = useMemo(() => {
    const years = Array.from(new Set(articlesWithDate.map((article) => article._year)))
  ...
  const filteredArticles = useMemo(() => {
    return articlesWithDate.filter((article) => {
      if (selectedYear !== 'all' && article._year !== Number(selectedYear)) return false
      if (selectedMonth !== 'all' && article._monthIndex !== selectedMonth) return false
      return true
    })
    ...
    <main className="bg-sand-50 text-ink-900">
      <section className="mx-auto max-w-6xl px-6 py-20 lg:px-10">
        ...
        <p className="mt-6 max-w-3xl text-lg text-ink-700">
          Browse updates, views, and events from the Centre for Sexualities, AIDS & Gender.
        </p>
        <p className="mt-3 text-sm text-ink-600">{articles.length} articles available.</p>
        <div className="mt-8 flex flex-wrap items-end gap-4">
          ...
          <div>
            <label className="text-xs uppercase tracking-[0.3em] text-ink-600">Month</label>
            <select
              className="mt-2 w-40 rounded-2xl border border-ink-900/10 bg-white px-4 py-2 text-sm"
              value={selectedMonth}
              onChange={(event) => {
            ...
            >
              ...
              {availableMonths.map((monthIndex) => {
                const monthName = new Date(`2020-${monthIndex}-01`).toLocaleString('en-US', { month: 'long' })
                return (
          ...
          <div className="text-sm text-ink-600">
            Showing {filteredArticles.length === 0 ? 0 : pageStart + 1}-{Math.min(pageEnd, filteredArticles.length)} of{' '}
          ...
          {groupOrder.map((group) => (
            <section key={`${group.year}-${group.month}`}>
              <div className="mb-5 flex items-center justify-between">
                <div>
                  <p className="text-xs uppercase tracking-[0.3em] text-ink-600">{group.year}</p>
                  <h2 className="font-display text-3xl text-ink-950">{group.month}</h2>
                </div>
                <p className="text-sm text-ink-600">{group.items.length} articles</p>
              </div>
              <div className="grid gap-6 lg:grid-cols-2">
                {group.items.map((article) => (
                  ...
                  >
                    ...
                    <h3 className="mt-4 font-display text-2xl text-ink-950">{article.title}</h3>
                    {article.author && <p className="mt-2 text-sm text-ink-600">By {article.author}</p>}
                    {article.featuredImage && (
                      <div className="mt-5 overflow-hidden rounded-2xl border border-ink-900/10">
                        <img src={article.featuredImage} alt="" className="h-40 w-full object-cover" loading="lazy" />
                      </div>
                    )}
                    {article.contentText && (
                      <p className="mt-5 text-sm text-ink-700">{getExcerpt(article.contentText)}</p>
                    )}
                    <div className="mt-6">
                      <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-950/70 px-6 py-10">
          <div className="relative max-h-[90vh] w-full max-w-4xl overflow-hidden rounded-3xl bg-white shadow-lift">
            <div className="flex items-center justify-between border-b border-ink-900/10 px-6 py-4">
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-ink-600">
                  {activeArticle.categories?.[0] || 'News'} - {activeArticle.dateLabel || 'Date pending'}
                </p>
                <h2 className="mt-2 font-display text-2xl text-ink-950">{activeArticle.title}</h2>
              </div>
              <button
            ...
            <div className="max-h-[70vh] overflow-y-auto px-6 py-6">
              {activeArticle.author && <p className="text-sm text-ink-600">By {activeArticle.author}</p>}
              {activeArticle.featuredImage && (
                <div className="mt-5 overflow-hidden rounded-2xl border border-ink-900/10">
                  <img src={activeArticle.featuredImage} alt="" className="h-64 w-full object-cover" loading="lazy" />
                </div>
              )}
              {activeArticle.contentHtml ? (
                <div
                  className="article-content mt-6 max-w-none text-ink-800"
                  dangerouslySetInnerHTML={{ __html: normalizeHtml(activeArticle.contentHtml) }}
                />
              ) : (
